...
abstract class TweetSet extends TweetSetInterface {
  ...
  def union(that: TweetSet): TweetSet = {
    that.filterAcc(tweet => true, this)
  }
  ...
  def mostRetweeted: Tweet = {
    // assuming there is no such value (-1) for retweets in any scenario
    mostRetweetedAcc(new Tweet("", "", -1))
  }
  ...
  def descendingByRetweet: TweetList = {
    def descendingByRetweetAux(set: TweetSet, list: TweetList): TweetList = {
      if (set.isEmpty) list
      else {
        val max = set.mostRetweeted
        new Cons(max, descendingByRetweetAux(set.remove(max), list))
      }
    }

    descendingByRetweetAux(this, Nil)
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  // purposefully commented out - this is my initial implementation of mostRetweeted in order for you to see -
  // but I have decided to go with an accumulator method instead :)
  //  def mostRetweeted: Tweet = {
